        ...
        <section className="w-full bg-gray-100 dark:bg-gray-900 py-12 xl:py-20 flex items-center justify-center">
            <div className="w-full max-w-4xl px-4 xl:px-0 text-center">
                ...
                <CollapsiblePanel title="Experience" defaultOpen={false}>
                    <div className="flex flex-col gap-8">
                        {experience.map((exp, idx) => (
                            <div key={exp.title + exp.company} className="bg-gray-50 dark:bg-gray-800 rounded-lg shadow p-6">
                ...
                <CollapsiblePanel title="Education" defaultOpen={false}>
                    <div className="flex flex-col gap-8">
                        {education.map((edu) => (
                            <div key={edu.degree + edu.institution} className="bg-gray-50 dark:bg-gray-800 rounded-lg shadow p-6">
                                <div className="flex flex-col md:flex-row md:justify-between md:items-center mb-2">
                                    <div>
                                        <span className="font-bold text-lg text-gray-800 dark:text-white">{edu.degree}</span>
                                        <span className="ml-2 text-gray-600 dark:text-gray-300">@ {edu.institution}</span>
                                    </div>
                                    <span className="text-sm text-gray-500 dark:text-gray-400">{edu.period} | {edu.location}</span>
                                </div>
                                {edu.description && (
                                    <p className="text-gray-700 dark:text-gray-200 mt-2">{edu.description}</p>
                                )}
                            </div>
                        ))}
                    </div>
